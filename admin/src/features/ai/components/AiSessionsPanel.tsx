...
import { AiTranscriptSheet } from './AiTranscriptSheet';

/**
 * Conversations the public site's Uyiz AI assistant has held.
 *
 * Lifted out of `/ai/page.tsx` unchanged when that page grew the assistant's
 * settings and spend above it: same query, same columns, same transcript
 * sheet. It is a MODERATOR read, which is why it is the one block on the page
 * everybody who can open /ai can see.
 *
 * `GET /admin/ai/sessions` is pagination and nothing else — no search, no date
 * range, no filter by user — so this panel has no FilterBar. An empty one would
