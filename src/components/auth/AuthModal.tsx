...
export const AuthModal: React.FC = () => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await ApiService.sendOtp(phone.trim());
      if (res.status === 'success') {
         setAuthStep('OTP');
         if (res.devCode) console.log("DEV OTP CODE:", res.devCode);
      } else {
         setError(res.message || "Xatolik yuz berdi");
      }
    } catch (err: any) {
      setError(err?.message || "SMS yuborishda xatolik yuz berdi.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const user = await ApiService.verifyOtp(phone.trim(), otpCode.trim(), activeTab === 'REGISTER' ? name.trim() : undefined);
      if (activeTab === 'REGISTER' && role) user.role = role;
      triggerSuccessLogin(user, 'Muvaffaqiyatli avtorizatsiya');
    } catch (err: any) {
        ...
        {successUser ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 p-1 bg-slate-100 rounded-2xl mb-5 text-sm font-bold">
              <button
                type="button"
                onClick={() => { setActiveTab('LOGIN'); setError(''); setAuthStep('PHONE'); setOtpCode(''); }}
              ...
              <button
                type="button"
                onClick={() => { setActiveTab('REGISTER'); setError(''); setAuthStep('PHONE'); setOtpCode(''); }}
                className={`py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-all ${
            ...
            {/* TAB 1: KIRISH (LOGIN) */}
            {activeTab === 'LOGIN' && (
              <form onSubmit={authStep === 'PHONE' ? handleSendOtp : handleVerifyOtp} className="space-y-4">
                {authStep === 'PHONE' ? (
                  <>
                    <button
                      type="button"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">SMS tasdiqlash kodi</label>
                    <input
                      type="text"
                      value={otpCode}
                      ...
                      placeholder="0000"
                      maxLength={4}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-base font-semibold text-slate-900 text-center tracking-[0.5em] focus:bg-white focus:border-emerald-600 outline-none transition-colors"
                    />
                    <button type="button" onClick={() => {setAuthStep('PHONE'); setError('');}} className="text-xs text-emerald-600 font-semibold mt-2 hover:underline">
                ...
                >
                  {busy ? 'Kutib turing...' : authStep === 'PHONE' ? 'SMS kod yuborish' : 'Kirish'}
                </button>
              </form>
            )}
            ...
            {activeTab === 'REGISTER' && (
              <div>
                {!role ? (
                  ...
                  <form onSubmit={authStep === 'PHONE' ? handleSendOtp : handleVerifyOtp} className="space-y-3">
                    {authStep === 'PHONE' ? (
                      <>
                    ...
                    <button
                      ...
                      className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black text-base py-4 rounded-xl shadow-lg shadow-emerald-600/20 active:scale-[0.98] transition-all"
                    >
                      {busy ? 'Kutib turing...' : authStep === 'PHONE' ? 'SMS kod yuborish' : 'Ro\'yxatdan o\'tish'}
                    </button>
                  </form>
                )}
              </div>
